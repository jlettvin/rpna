...
1. Run it as a standalone aggregate calculator app using RPN.
   ...
   c) execute code
      ...
      - `[8]\ *`
      - `[9]\ show`
      - '[10]\ :square|pop|push|push|*|"square the args"
      - `[10]\ (2,square,show)`
...
class RPN(object):
    ...
    def __call__(self, source, **kw):
        ...
        self.set_symbols()
        self.set_source_planes(source)
        self.set_target_planes()            # <<< 2nd BIGGEST CONSUMER

        # read codefile every time to pick up changes dynamically.
        filename = kw['filename'] = kw.get('rpn', 'capture.rpn')
        self.interpret_load('!', filename)  # <<< 1st BIGGEST CONSUMER
        self.first = False

        # return generated target array or source array to Capture.py
        if self.ready:
            dx, dy = self.kernelX, self.kernelY
            self.target = self.symbol[-1].get('target', source)[dx:-dx, dy:-dy]
            # TODO What is done with self.target?
            assert 'bad place' == 'here'
        else:
